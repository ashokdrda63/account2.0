...
function VoucherEntey() {
  ...
  console.log("editable77777777777777",voucherRows);
 ...
 const [disable,setDisable] = useState(false)
  ...
  function createVoucherRow(accountTy, accountDesName, accountTo, accountFrom,newVoucher,voucherDate,selectAccount) {
    const parsedAccountTo = parseFloat(accountTo);
    const parsedAccountFrom = parseFloat(accountFrom);
    let vch_type =newVoucher;
    let vch_date =voucherDate.toISOString().split('T')[0];
    let acc_id =selectAccount.accountId !=="" && selectAccount.accountId;
  ...
  const onEnterPress = (e) => {
    if (e.key === "Enter") {
      let newRow = createVoucherRow(accountType, selectedAccountName, accountTo, accountFrom,newVoucher,voucherDate,selectAccount)
